...
class _StatusPageState extends State<StatusPage> {
  ...
  bool get _hasNavData {
    ...
    return nav.status != 'IDLE' || nav.progress > 0.0 || nav.distanceRemain > 0.0;
  }
  ...
  bool get _hasTaskData {
    ...
    return task.taskId.isNotEmpty || task.status != 'PENDING' || task.totalSteps > 0;
  }
          ...
          ? Column(
              children: [
                _buildDetailRow(
                  ...
                  obs.isObstacle ? Icons.warning_amber : Icons.check_circle_outline,
                  obs.isObstacle ? AppColors.warningOrange : AppColors.successGreen,
                ),
  ...
  Widget _buildSystemInfo() {
    return AppCard(
      ...
      child: Column(
        children: [
          _buildDetailRow('主控', 'Jetson Orin Nano', Icons.developer_board, AppColors.darkNavy),
          const Divider(height: 20),
          _buildDetailRow('系统', 'Ubuntu 20.04', Icons.laptop, AppColors.darkNavy),
          const SizedBox(height: 8),
          _buildDetailRow('ROS2', 'Foxy', Icons.hub, AppColors.darkNavy),
          const SizedBox(height: 8),
          _buildDetailRow('驱动模式', '麦轮全向', Icons.all_inclusive, AppColors.bluePurple),
        ],
      ),
    );
  }
