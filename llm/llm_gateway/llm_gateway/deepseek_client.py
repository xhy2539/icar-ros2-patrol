...
如果用户只是在询问信息（状态、位置、看到了什么、有没有障碍物等），
...
当用户只是询问信息（不是下达任务命令）时，输出：{
  "task_type": "info",
  "query_type": "vision|navigation|safety|status|all",
  "question": "用户的问题原文",
  "params": {"source": "llm_gateway", "provider": "deepseek", "raw_text": "原始输入"}
}
...
【字段说明 - 信息查询】
- task_type: 固定为 "info"
- query_type: vision(视觉检测结果), navigation(位置/导航进度), safety(障碍物/安全风险), status(任务状态), all(综合)
- question: 用户的问题原文

【动作识别规则】
- 用户提到"检测"/"识别"/"目标"/"视觉"/"拍照" → 包含 detect_object
- 用户提到"采集"/"传感器" → 包含 collect_sensor 动作
- navigate 和 avoid_obstacle 始终包含（patrol任务）
- 用户说"巡检"或"一圈" → 包含全部4个动作

【安全规则识别】
- 提到"障碍"/"避障" → "遇到障碍物停止并等待处理"
- 提到"报警"/"异常" → "检测到异常时停止并报警"
- 提到"停止" → "收到停止指令立即停止"
...
信息查询类（不需要参数）：
6. query_vision() - 查询最近的视觉检测结果（看到了什么）
7. query_navigation() - 查询当前导航状态和位置
8. check_safety() - 查询障碍物状态和安全风险
...
class DeepSeekClient:
    ...
    def answer_query(self, question: str, context: Dict[str, Any]) -> Optional[str]:
        ...
        context 应包含: nav_status, obstacle_status, task_status, detections
        """
