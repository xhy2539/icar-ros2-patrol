...
touch "$LOG" 2>/dev/null || LOG=/home/jetson/icar_guard.log
...
ensure_single() {
  ...
  eval "docker exec $container bash -lc 'source /opt/ros/foxy/setup.bash; source /root/icar_ros2_ws/icar_ws/install/setup.bash; export ROS_DOMAIN_ID=30; nohup $* </dev/null >/dev/null 2>&1 &'" 2>/dev/null || true
}
...
# ── Fix retained online ──
docker exec icar_ros2 python3 -c "
import paho.mqtt.client as mqtt, json; c=mqtt.Client(callback_api_version=mqtt.CallbackAPIVersion.VERSION2)
c.username_pw_set('icar','icar123456'); c.connect('82.156.132.43',1883,60)
c.loop_start(); import time; time.sleep(1)
c.publish('/icar/online', json.dumps({'online':True}), qos=1, retain=True)
